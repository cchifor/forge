...
- **`docs/auth-architecture.md` new** — architectural reference: TL;DR, four roles of Gatekeeper, per-language SDK design parity table, three token-flow walkthroughs (browser login, S2S on-behalf-of, inactivity-driven session refresh), 16-entry configuration-knobs table, compliance notes (SOC 2 / ISO 27001 / NIST 800-63B-4 / OWASP / RFC 8693 / RFC 9068), Cutover-status section explicitly enumerating the Wave 1 (shipped: 6 of 11 fragments, SDKs + frontend session-timeout) vs Wave 2 (pending: backend middleware + gatekeeper compose cutover) split, migration overview, out-of-scope follow-ups (step-up auth, IETF SSE/CAEP, risk-adaptive timeouts, concurrent-session limits, mobile native refresh, alternative `KEY_BACKEND`s, shared SPA SDK).
