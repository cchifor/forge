...
| Flutter + chat e2e (`test_flutter_full_analyzes`) uses local canvas overrides | The generated Flutter app targets a modern toolchain — its deps (`flutter_lints ^6`, `riverpod ^3`, `freezed ^3`, `go_router ^17`) floor at Dart `^3.8.0`, so the SDK env is `>=3.8.0 <4.0.0` / `flutter >=3.32.0` and the e2e job installs the latest **stable** Flutter (was pinned to 3.24.x, which shipped Dart 3.5.4 and failed version solving). | The unpublished `forge_canvas`/`forge_canvas_core` are handled by a `dependency_overrides` `path:` block in the test, so `flutter pub get` + `flutter analyze` run against in-repo source. | RFC-003 — publish the canvas packages to pub.dev to retire the overrides. |
