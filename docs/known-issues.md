...
| Flutter + chat e2e (`test_flutter_full_analyzes`) is `xfail` | The generated Flutter app targets a modern Flutter — its deps (`flutter_lints ^6`, `riverpod ^3`, `freezed ^3`, `go_router ^17`) floor at Dart `^3.8.0` / Flutter 3.32+ — but the e2e job pins `flutter-version: 3.24.x` (Dart 3.5.4), so `flutter pub get` fails version solving. (The earlier bogus `sdk: ^3.10.4` env pin — Dart 3.10 doesn't exist — is corrected to `>=3.8.0 <4.0.0`, `flutter >=3.32.0`, consistent with those deps.) | The unpublished `forge_canvas`/`forge_canvas_core` are already handled by a `dependency_overrides` `path:` block in the test; the only remaining blocker is the CI Flutter version. | WS-4.4 — bump the e2e job's `flutter-version` to `>=3.32` (Dart `>=3.8`) and drop the `xfail`; publish the canvas packages to pub.dev to retire the overrides. |
