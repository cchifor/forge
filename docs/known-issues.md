...
| Flutter + chat e2e (`test_flutter_full_analyzes`) needs local canvas overrides | The generated Flutter chat app depends on `forge_canvas`/`forge_canvas_core`, not yet on pub.dev. | The test injects a `dependency_overrides` block pointing both at `packages/forge-canvas{,-core}-dart/` via `path:`, so `flutter pub get` + `flutter analyze` run against in-repo source. | RFC-003 / WS-4.4 — unneeded once the canvas packages ship to pub.dev. (The bogus `sdk: ^3.10.4` env pin that previously broke version solving — Dart 3.10 doesn't exist; CI Flutter 3.24 ships Dart 3.5.4 — is fixed to `>=3.5.0 <4.0.0`, `flutter >=3.24.0`.) |
