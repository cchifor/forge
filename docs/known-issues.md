...
| --- | --- | --- | --- |
| Svelte + chat e2e (`test_svelte_chat_on_typechecks`) is `xfail` | The generated svelte chat frontend pins `@forge/canvas-core` as `workspace:*` (unpublished), so a standalone `npm install` (outside the forge workspace) can't resolve it. `svelte-check` needs `node_modules`, so — unlike `vue-tsc`, which tolerates the missing module without an install — it can't typecheck. The test runs in the chat e2e lane but is expected-fail. | Build `@forge/canvas-core` (`npm run --workspace=@forge/canvas-core build`) and rewrite the dep to a `file:` path before `npm install`. The Vue chat path is covered. | WS-5.2 — publish `@forge/canvas-core` to npm + pin a real version (or build it in the chat e2e job and rewrite to `file:`), then drop the `xfail`. |
| Flutter + chat e2e (`test_flutter_full_analyzes`) is `xfail` on analyzer null-safety debt | The generated Flutter app targets a modern toolchain (deps `flutter_lints ^6`, `riverpod ^3`, `freezed ^3`, `go_router ^17` floor at Dart `^3.8.0`; SDK env `>=3.8.0 <4.0.0` / `flutter >=3.32.0`; the e2e job now installs `channel: stable`). With a modern Dart, `flutter pub get` resolves and `flutter analyze` runs — but reports ~500 `unchecked_use_of_nullable_value` errors across ~46 files (templates access nullable API-response fields like `response.title` / `data['x']` without `?.`/`!`). The old 3.24.x pin (Dart 3.5.4) never ran analyze far enough to flag these. | The unpublished `forge_canvas`/`forge_canvas_core` are handled by a `dependency_overrides` `path:` block in the test, so the toolchain runs against in-repo source. The analyzer findings are real template bugs. | A template null-safety sweep across the flutter-frontend-template `lib/src/**` widgets/controllers (add `?.`/`!`/null-guards on API-model access), then drop the `xfail`. RFC-003 — publish canvas packages to pub.dev to retire the overrides. |
