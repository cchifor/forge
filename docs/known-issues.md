...
| --- | --- | --- | --- |
| Full-feature golden preset (`platform.admin` + `agent.llm` + `rag.reranker` enabled) — **likely fixed** (2026-05-24). Investigation traced all three options' fragments and found none inject into `container.py`; the original crash was likely caused by the pre-fix `error_port` targeting (fixed in 9cb667e) or pre-PortSpec injection paths. A `full_feature_max` golden-snapshot preset now exercises this combination in CI. | Pending CI verification. If the preset passes, this row can be removed. | `tests/test_golden_snapshots.py::PRESETS['full_feature_max']` exercises the combination. | P0.2 production-readiness plan. |
