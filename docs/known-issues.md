...
| --- | --- | --- | --- |
| Svelte + chat e2e (`test_svelte_chat_on_typechecks`) is `xfail` | Canvas is now **vendored** into the generated project (`src/lib/features/chat/canvas-core/`, aliased via `kit.alias`), so `npm install` + the production `build` both succeed self-contained — no `workspace:*`/registry dependency. `svelte-check` still reports ~7 **type** mismatches between the svelte chat scaffolding's hand-written `chat.types.ts`/components and canvas-core's current exported API (e.g. `UserPromptPayload.tool_call_id`, `AppBridge.teardownResource`, MCP-bridge option shapes). The scaffolding was written against an older canvas-core API; the build tolerates it but `svelte-check` (the e2e gate) does not. | The app installs + builds; only the type-check is red. Vue's chat path is `svelte-check`-equivalent clean (vue-tsc green). | Reconcile the svelte chat template types against canvas-core's API, then drop the `xfail`. (Vendoring already removed the old `workspace:*` blocker — no publish needed.) |
