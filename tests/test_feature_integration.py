...
class TestManifestDiscovery:
    def test_all_manifests_discovered(self) -> None:
        ...
        assert len(manifests) == 27, (
            f"Expected 27 feature manifests, discovered {len(manifests)}: "
...
class TestLoadAll:
    def test_load_all_populates_loaded_features(self) -> None:
        """load_all() discovers and loads all 27 built-in features."""
        result = feature_loader.load_all()
        assert len(result) == 27
        assert len(feature_loader.LOADED_FEATURES) == 27
