...
class TestLoadAll:
    def test_load_all_populates_loaded_features(self) -> None:
        """load_all() discovers and loads all 22 built-in features."""
        result = feature_loader.load_all()
        assert len(result) == 22
        assert len(feature_loader.LOADED_FEATURES) == 22
