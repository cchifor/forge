...
class TestManifestDiscovery:
    def test_all_25_manifests_discovered(self) -> None:
        ...
        assert len(manifests) == 26, (
            f"Expected 26 feature manifests, discovered {len(manifests)}: "
...
class TestLoadAll:
    def test_load_all_populates_loaded_features(self) -> None:
        """load_all() discovers and loads all 26 built-in features."""
        result = feature_loader.load_all()
        assert len(result) == 26
        assert len(feature_loader.LOADED_FEATURES) == 26
