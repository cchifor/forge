...
def _write_forge_toml(
    ...
    Captures the forge version that scaffolded the project, per-template paths,
    and the enabled feature set. Lets `copier update` (or a future `forge update`
    subcommand) figure out what to re-render after a template change.
    """
    ...
    features_section = ""
    if plan is not None and plan.ordered:
        feature_keys = ", ".join(f'"{f.spec.key}"' for f in plan.ordered)
