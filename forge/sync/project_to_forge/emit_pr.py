...
def emit_pr(
    ...
    *,
    ...
    # ----- Cross-lang-only bundle guard --------------------------------------
    # RFC-006 cross-language suggestions are informational only — they
    # have no diff to apply, so a bundle that carries ONLY suggestions
    # has nothing to commit. Refuse early rather than create an empty
    # branch and a no-op PR.
    real_candidates = [c for c in bundle.candidates if c.kind != "cross-lang-suggest"]
    if not real_candidates and bundle.candidates:
        return EmitPrReport(
            bundle_id=bundle_id,
            forge_repo=forge_repo,
            branch_name="",
            entries=(),
            errors=("bundle contains only cross-lang suggestions; nothing to commit",),
        )
...
def _process_candidate(
    *,
    ...
    # RFC-006 cross-language suggestions are informational only —
    # they're surfaced in the PR body's reviewer checklist (see
    # ``_default_pr_body``) and never produce a commit. Defer here
    # before the risk filter so the entry's ``risk`` doesn't muddy the
    # report (suggestions are stamped ``needs-review`` but they bypass
    # the filter by design).
    if cand.kind == "cross-lang-suggest":
        return EmitPrEntry(
            ...
            files_touched=(),
            action="deferred",
            reason="cross-lang suggestion; surfaced in PR body reviewer checklist",
        )
...
def _default_pr_body(
    ...
    *,
    ...
    # RFC-006 cross-language parity hints — every ``cross-lang-suggest``
    # candidate in the bundle becomes a checklist item naming the
    # sibling impl the maintainer must mirror by hand. The original
    # block candidate that triggered the suggestion is already covered
    # by the commit; this surface is the "and also" prompt.
    suggestion_lines = _cross_lang_checklist_lines(bundle)
    if suggestion_lines:
...
def _cross_lang_checklist_lines(bundle: HarvestBundle) -> list[str]:
    """Return reviewer-checklist lines for ``cross-lang-suggest`` candidates.

    Empty list when the bundle has no suggestions, so the caller can
    skip the section header cleanly. Suggestions are de-duplicated by
    (fragment, backend, target_path) — a bundle that surfaces the same
    sibling impl from two different block candidates still produces one
    line per pair.
    """
    ...
    for cand in bundle.candidates:
        if cand.kind != "cross-lang-suggest":
            continue
        key = (cand.fragment, cand.backend, cand.target_path)
        if key in seen:
            continue
        seen.add(key)
        lines.append(
            f"  - [ ] Mirror the change to {cand.backend} impl at {cand.target_path} "
            f"(fragment {cand.fragment!r})"
        )
    return lines
