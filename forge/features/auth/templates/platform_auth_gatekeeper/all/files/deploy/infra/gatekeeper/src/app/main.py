...
from app.gatekeeper.service_token import router as gatekeeper_service_token_router
...
def _configure_routers(app: FastAPI) -> None:
    ...
    # Gatekeeper-issued JWT public keys (/auth/jwks).
    app.include_router(gatekeeper_jwks_router)
    # Service-to-service token endpoint (/auth/token).
    app.include_router(gatekeeper_service_token_router)
    # BFF session countdown (GET) + extension (POST) on /auth/session.
    app.include_router(gatekeeper_session_router)
