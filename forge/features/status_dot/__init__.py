"""StatusDot — a Layer-1 pure-UI canvas component (status indicator dot).

A *component* feature: ``feature.toml`` declares ``[feature].layer = 1`` and the
loader auto-registers the emitter fragment (``component_StatusDot``) from the
manifest, copying this feature's ``templates/component_StatusDot/all/files`` tree
