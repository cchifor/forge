...
``mcp_template.*`` hosts a first-party MCP server inside this service via
a vendored, self-contained template (no private SDKs).
"""
...
def register_all(api: ForgeAPI) -> None:
    api.add_option(
        Option(
            ...
            default=False,
            summary="Host a first-party MCP server inside this service (vendored).",
            description="""\
