...
_VALID_AGENT_TRANSPORTS: frozenset[str] = frozenset({"external", "stub"})


def emit_capabilities(agent_transport: str) -> str:
    """Emit the generated ``capabilities.ts`` flag (plan §F).

    ``agent_transport`` is ``"external"`` when the project binds an agent
    (``subscribe``-kind) contract op to the upstream backend, ``"stub"``
    otherwise. The chat component reads ``agentTransport`` at mount; ``"stub"``
    renders an inert "agent not available" surface rather than opening a live
    transport. Regenerated on every ``--update``, so flipping a binding on and
    re-running flips the panel live — it is not runtime-polled.
    """
    if agent_transport not in _VALID_AGENT_TRANSPORTS:
        raise GeneratorError(
            f"Unknown agent transport {agent_transport!r}; "
            f"expected one of {sorted(_VALID_AGENT_TRANSPORTS)}."
        )
    return (
        "// Generated by forge (brownfield capabilities). Do not edit by hand.\n"
        f'export const agentTransport = "{agent_transport}" as const;\n'
    )
