...
fetch is triggered on first use, when the cache is older than ``lifespan``, or
when a token presents a ``kid`` we have never seen (most likely a key
rotation). On upstream failure the cache serves the last known good document
for up to ``stale_max`` seconds so a transient IdP outage does not flatline
every service.
...
Constructed once per process (typically inside :class:`AuthGuard`); never per
request.
"""
...
from forge_core.security.exceptions import InvalidToken
...
class JWKSCache:
    ...
    def register_issuer(self, issuer: str, jwks_uri: str) -> None:
        """Register an issuer's JWKS URI (idempotent)."""
        if not issuer:
    ...
    async def _fetch(self, jwks_uri: str) -> dict[str, pyjwt.PyJWK]:
        ...
        for raw in keys:
            ...
            # signing key by default); a single bad entry must not fail the
            # whole fetch.
            use = raw.get("use")
            ...
            try:
                ...
                _log.warning("jwks_key_skipped", extra={"kid": kid, "error": str(exc)})
        if not result:
