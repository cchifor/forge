...
class TMSConfig(BaseModel):
    ...
    keycloak_admin_url: str = Field("http://keycloak:8080", min_length=10)
    keycloak_admin_user: str = Field("admin", min_length=1)
    keycloak_admin_password: str = Field("admin", min_length=1)
    redis_url: str = Field("redis://redis:6379", min_length=10)
    default_gatekeeper_client_id: str = "gatekeeper"
    keycloak_ssl_required: str = "none"  # "none" for dev, "external" for prod
