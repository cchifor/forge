"""Pinecone-backed RAG endpoints at /api/v1/rag/pinecone.

Uses Pinecone namespaces for tenant isolation — `customer_id` routes to a
dedicated namespace per tenant. The index itself must exist before first
write; create it via the Pinecone console or SDK matching your
`EMBEDDING_DIM`.
"""
...
from app.rag import pinecone_backend
...
async def ingest(
    ...
    count = await pinecone_backend.store_chunks(
    ...
    return {"document_id": str(document_id), "chunks_created": count, "backend": "pinecone"}
...
async def search(
    ...
    hits = await pinecone_backend.search(vector, top_k=top_k, customer_id=cid)
    return {
        "results": [
            {
