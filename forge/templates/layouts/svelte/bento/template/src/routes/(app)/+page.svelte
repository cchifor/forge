<script lang="ts">
	import { Activity, Server, Info, Package, User } from 'lucide-svelte';
	...
	import BentoGrid from '$lib/features/shell/ui/BentoGrid.svelte';
...
<div class="space-y-6">
	<div>
		...
		<p class="text-muted-foreground">A bento overview of your service</p>
	</div>

	<!-- Bento mosaic: tiles compose inside the BentoGrid region. -->
	<BentoGrid>
		<!-- Service Card (tall) -->
		<div class="row-span-2 flex flex-col rounded-lg border bg-card text-card-foreground shadow-sm">
		...
		<!-- Quick Actions Card (wide) -->
		<div class="col-span-1 rounded-lg border bg-card text-card-foreground shadow-sm sm:col-span-1 lg:col-span-1">
			<div class="flex flex-row items-center justify-between space-y-0 p-6 pb-2">
		...
		<!-- Health Components (wide, tall) -->
		{#if $readinessQuery.data?.components}
			<div class="col-span-1 row-span-2 rounded-lg border bg-card text-card-foreground shadow-sm sm:col-span-2 lg:col-span-2">
				<div class="p-6">
		...
		{#if $readinessQuery.data?.system_info}
			<div class="col-span-1 rounded-lg border bg-card text-card-foreground shadow-sm sm:col-span-2 lg:col-span-4">
				<div class="p-6">
