...
<script lang="ts">
  ...
  let layout = $derived.by(() => {
    ...
    for (const [d, ids] of byDepth) {
      ...
      ids.forEach((id, i) => {
        positions.set(id, { x: startX + i * (NODE_W + H_GAP), y: d * (NODE_H + V_GAP) })
      })
    }
...
<div class="forge-canvas-wf">
  <svg viewBox="0 0 {layout.width} {layout.height}" style="width: 100%; height: {layout.height}px; max-height: 600px">
    <defs>
    ...
    <g>
      {#each nodes as node (node.id)}
        <g transform="translate({layout.positions.get(node.id)?.x ?? 0}, {layout.positions.get(node.id)?.y ?? 0})">
          <rect class="forge-canvas-wf__node {statusClass(node.status)}" width={NODE_W} height={NODE_H} rx="8" ry="8" />
          <text x={NODE_W / 2} y={NODE_H / 2 + 5} text-anchor="middle" class="forge-canvas-wf__label">
